...
const WorkerHub = () => {
  ...
  const filterWorkers = () => {
    ...
    if (location) {
      filteredWorkers = filteredWorkers.filter((worker) => {
        return worker.location?.toLowerCase().includes(location.toLowerCase());
      });
    }
    ...
    <div className="bg-gray-50 min-h-screen">
      ...
      <div className="container mx-auto p-6 flex gap-8">
        ...
        <div className="w-3/4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
            {workers.map((worker) => (
              <div key={worker.id} className="bg-white p-6 rounded-lg shadow-lg flex flex-col justify-between items-start hover:shadow-xl transition duration-300">
                <h3 className="text-xl font-semibold text-gray-800">{worker.name}</h3>
                <p className="text-sm text-gray-600">{worker.service}</p>
                <p className="text-sm text-gray-500">{worker.address}</p>
                <p className="text-sm text-gray-500">{worker.description}</p>
